process AMRFINDERPLUS_RUN {
    ...
    conda "conda::ncbi-amrfinderplus=4.2.7"
...
//     // check if the current meta.organism is in the user-provided list
//     def organisms_list   = organisms.tokenize(',')
//     def current_org      = meta.organism ?: ""
    
    
//     // apply the flag if the organism is in our targeted parameter list
//     def organism_param = organisms_list.contains(current_org) ? "--organism ${current_org}" : ""
...
//         # 2. Specialized Mutation Run (Only if organism matches your list)
//         if [ -n "${organism_param}" ]; then
//             amrfinder \\
//                 ${fasta_param} "\$fasta" \\
//                 ${organism_param} \\
//                 --mutation_all "tmp_\${fasta_id}_mut.tsv" \\
//                 --database \$DB_PATH --threads ${task.cpus} ${args} > /dev/null
//         fi
