...
process NCBI_DOWNLOAD_BATCH {
    ...
    conda "conda-forge::ncbi-datasets-cli=18.24.0"
...
//     input:
//     path(taxon_input)       // Path (e.g., params.pathogen_taxid)
...
//     # categorize as either Bacteria, Viruses 
//     touch accessions.txt

//     while read -r taxid; do

//         # Fetch the summary for this specific TaxID
//         datasets summary taxonomy taxon \$taxid --as-json-lines > report.jsonl

//         if grep -oE '"name":"Bacteria"' report.jsonl; then
//             TOGGLE="--reference"
//         else
//             TOGGLE=""
//         fi
        
//         # Extract only single accessions (head -n 1)
//         datasets summary genome taxon \$taxid \$TOGGLE --report sequence --as-json-lines | \\
//             dataformat tsv genome-seq --fields accession | \\
//             tail -n +2 | uniq | head -n 1 >> accessions.txt
