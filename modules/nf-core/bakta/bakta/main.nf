process BAKTA_BAKTA {
    ...
    path db
    path proteins,    stageAs: 'proteins.faa' 
    path prodigal_tf, stageAs: 'prodigal_tf'
    path regions,     stageAs: 'regions.gff3'
    path hmms,        stageAs: 'custom.hmm'
    ...
    def proteins_opt    = file('proteins.faa').exists() ? "--proteins proteins.faa" : ""
    def prodigal_tf_opt = file('prodigal.tf').exists() ? "--prodigal-tf prodigal.tf" : ""
    def regions_opt     = file('regions.gff3').exists() ? "--regions regions.gff3" : ""
    def hmms_opt        = file('custom.hmm').exists() ? "--hmms custom.hmm" : ""
